...
class ThesisPlotter:
    ...
    # 变量分类关键词
    GAS_KEYWORDS = ['CH4', 'CO2', 'N2O', 'VOCs', 'O2', 'H2S', '甲烷', '氧化亚氮']
    LIQUID_KEYWORDS = ['TOC（', 'TC(mg', 'IC(mg', 'COD（', 'DO(mg', '总氮', '铵态氮',
                       '硝态氮', '总磷', 'pH', '液温', '电导率', 'NaCl']
    SOLID_KEYWORDS = ['固总碳', '有机碳（g', '无机碳（g', 'DOC(mg/kg', '全磷（g',
                      '固）铵态氮', '固）硝态氮']
    ...
    def _find_cols(self, keywords, numeric_only=True):
        """根据关键词列表在DataFrame中查找匹配的列"""
        cols = []
        for kw in keywords:
            for col in self.df.columns:
                if kw in str(col) and col not in cols:
                    if numeric_only and not pd.api.types.is_numeric_dtype(self.df[col]):
                        continue
                    cols.append(col)
        return cols
    ...
    def plot_gas_boxplot(self):
        ...
        print("\n[生成图4] 气体浓度箱线图...")
        df = self.df
        gas_vars = self._find_cols(self.GAS_KEYWORDS)
        if not gas_vars:
            print("  警告: 气体数据不足")
            return
        if len(gas_vars) == 0:
    ...
    def plot_liquid_boxplot(self):
        ...
        print("\n[生成图5] 液相指标箱线图...")
        df = self.df
        liquid_vars = self._find_cols(self.LIQUID_KEYWORDS)
    ...
    def plot_correlation_heatmap(self):
        ...
        print("\n[生成图6] 相关性热图...")
        df = self.df
        corr_cols = self._find_cols(self.GAS_KEYWORDS + self.LIQUID_KEYWORDS)
        if len(corr_cols) < 5:
            print("  警告: 相关性分析数据不足")
            return
    ...
    def plot_pca_biplot(self):
        ...
        print("\n[生成图7] PCA双标图...")
        df = self.df
        pca_cols = self._find_cols(self.GAS_KEYWORDS + self.LIQUID_KEYWORDS)
        if len(pca_cols) < 5:
            print("  警告: PCA数据不足")
            return
    ...
    def plot_hca_dendrogram(self):
        ...
        print("\n[生成图8] 层次聚类树状图...")
        df = self.df
        hca_cols = self._find_cols(self.GAS_KEYWORDS + self.LIQUID_KEYWORDS)
        if len(hca_cols) < 5:
            print("  警告: HCA数据不足")
            return
