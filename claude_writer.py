...
Write directly."""
    ...
    def write_introduction(self, findings: list, domain: str = "污水管网碳排放",
        ...
        if language == "zh":
            prompt = f"""你是环境科学领域的学术论文写作引擎。直接输出论文正文，禁止任何元评论。

撰写一篇关于"{domain}"的中文学术论文的"引言"章节。

研究背景：城市污水管网系统是城市基础设施的重要组成部分，管道内的微生物活动可导致碳污染物在固、液、气三相之间发生相间迁移。污水管网同时承担碳污染物输送和生物转化的双重功能，是城市温室气体排放的重要来源。

本研究以某校园污水管网为对象，在教学区、生活区、餐饮区和管口出口设置10个采样点，分冬、春两季同步采集固相沉积物、液相污水和管内气相样品，测定CH₄、CO₂、N₂O、TOC、COD、TN等29项指标。

必须包含以下所有子章节，每个子章节必须有实质内容：

## 1.1 研究背景与意义
从全球气候变化→城市碳排放→污水管网碳排放递进，引用文献支撑。

## 1.2 国内外研究现状
综述污水管网碳污染物研究进展、校园污水管网研究现状，引用3-5篇文献。

## 1.3 现有研究不足
明确指出2-3个研究空白（多相态联合分析不足、校园尺度研究缺乏、碳平衡分析薄弱）。
...
撰写一篇关于"{domain}"的中文学术论文的"结论"章节。

研究背景：本研究以某校园污水管网为对象，分冬春两季同步采集固-液-气三相样品，测定29项指标，采用描述性统计、Mann-Whitney U检验、Pearson相关分析、PCA和HCA进行系统分析。
...
禁止输出：不要写元评论、交互文字、英文说明、"以下是"、"以上为"、"请提供"。只输出结论正文。
...
撰写一篇关于"{domain}"的中文学术论文的"材料与方法"章节。

研究概况：本研究以某校园污水管网为研究对象，占地面积约X公顷，常住人口约X万人。在主管道上设置了10个采样点，分别位于教学区(A1-A3)、生活区(B1-B3)、餐饮区(C1-C3)和管口出口(D)。采样时间涵盖冬季(2024年12月)和春季(2025年3月)两个季节，每次采样在各点同步采集固相、液相和气相样品。

数据概况:
- 样本数: {data_info.get('n_samples', '40')}
- 变量数: {data_info.get('n_variables', '29')}
- 变量列表: {', '.join(data_info.get('variables', [])[:20])}
- 分组: 冬季(20), 春季(20)
...
## 2.1 研究区域概况
描述校园污水管网的基本情况（面积、人口、功能分区）。

## 2.2 采样方案
描述采样点设置（10个点位）、采样时间（冬春两季）、采样方法（固液气三相同步采集）。

## 2.3 分析方法
描述气相（CH₄、CO₂、VOCs）、液相（TOC、COD、TN、TP）、固相（TC、TOC、IC）的分析方法，引用国家标准。

## 2.4 数据处理与统计分析
描述统计方法：描述性统计、Shapiro-Wilk正态性检验、Mann-Whitney U检验、Pearson相关分析、PCA、HCA。

写作规则：
1. 每个 ## 标题下必须有实质内容
2. 分析方法引用国家标准（如 TOC用HJ 501-2009，COD用GB 11914-89）
3. 统计方法要具体（说明软件、显著性水平等）
4. 语言要让同行能复现实验
5. 总字数1000-1500字

禁止输出：不要写元评论、交互文字、英文说明、"以下是"、"以上为"、"请提供"。只输出方法正文。
{info_text}
请直接输出方法正文，用 ## 标记子章节。"""
        else:
