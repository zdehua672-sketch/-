...
try:
    ...
    print(f"成功加载: {data_file} (Sheet: {sheet_name})")
...
# 统一采样点列名
if '检查井编号' in df.columns:
    df.rename(columns={'检查井编号': '采样点'}, inplace=True)

# 2.1 处理数值列
def clean_numeric(series):
    return pd.to_numeric(series.astype(str).str.replace('mg/L', '', regex=False), errors='coerce')
...
for col, label in gas_indicators.items():
    ...
    if col in df.columns:
        ...
        # 模糊匹配
        matches = [c for c in df.columns if col.split('(')[0] in c and '平均值' not in c]
        if matches: actual_col = matches[0]
    
    if actual_col:
        plot_data[label] = clean_numeric(df[actual_col])
        available_indicators.append(label)

# 2.2 VOCs 换算 (ppb -> ppm) 如果需要的话，这里保持原样或按需换算
# 既然之前的图用了 ppm，这里我们也换算一下
...
# 整体美化
plt.suptitle('春季各采样点气体浓度空间变化特征', fontsize=18, fontname='SimHei', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.97])

# 保存
output_png = r"C:\Users\Administrator\Desktop\spring_gas_spatial_distribution.png"
output_pdf = r"C:\Users\Administrator\Desktop\spring_gas_spatial_distribution.pdf"
plt.savefig(output_png, dpi=300, bbox_inches='tight')
plt.savefig(output_pdf, format='pdf', bbox_inches='tight')

print(f"Spring Gas Spatial plot saved to:\n- {output_png}\n- {output_pdf}")
plt.close()
